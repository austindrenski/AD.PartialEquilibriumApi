...
namespace AD.PartialEquilibriumApi
{
    ...
    public static class EnforceBoundsExtensions
    {
        ...
        public static double[] EnforceBounds(this double[] vector, Simplex simplex)
        {
            ...
            {
                vector[i] = vector[i] < simplex.LowerBound ? simplex.LowerBound + 0.10 * (simplex.UpperBound - simplex.LowerBound) : vector[i];
                vector[i] = vector[i] > simplex.UpperBound ? simplex.UpperBound - 0.10 * (simplex.UpperBound - simplex.LowerBound) : vector[i];
            }
            return vector;
        }
